...
class Trainer:
    ...
    def Dataset_Generate(self):
        ...
        inference_dataset = Inference_Dataset(
            ...
            sources= self.hp.Train.Inference_in_Train.Source
            )
        ...
        collater = Collater(
            pattern_length= self.hp.Train.Segment_Size,
    ...
    def Train_Step(self, features, audios):
        ...
        self.scaler.scale(loss_dict['Diffusion']).backward()
    ...
    def Train_Epoch(self):
        for features, audios in self.dataloader_dict['Train']:
            self.Train_Step(
                features= features,
                audios= audios
                )
    ...
    @torch.no_grad()
    def Evaluation_Step(self, features, audios):
        loss_dict = {}
        features = features.to(self.device, non_blocking=True)
        audios = audios.to(self.device, non_blocking=True)
        ...
        return predictions, noises, epsilons
    ...
    def Evaluation_Epoch(self):
        ...
        for step, (features, audios) in tqdm(
            ...
            predictions, noises, epsilons = self.Evaluation_Step(
                features= features,
                audios= audios,
                )
        ...
        if self.gpu_id == 0:
            ...
            index = np.random.randint(0, features.size(0))

            with torch.no_grad():
                prediction_audio, *_ = self.model(
                    conditions= features[index].unsqueeze(0).to(self.device)
                    )
            ...
            image_dict = {
                'Diffusion/Noise': (noises[index].cpu().numpy(), None, 'auto', None, None, None),
                'Diffusion/Epsilon': (epsilons[index].cpu().numpy(), None, 'auto', None, None, None),
                }
            audio_dict = {
    ...
    @torch.no_grad()
    def Inference_Step(self, features, feature_lengths, sources, start_index= 0, tag_step= False):
        features = features.to(self.device, non_blocking=True)
        ...
        audios = [
            ...
            for audio, length in zip(predictions, feature_lengths)
            ]
        ...
        for index, (
            ...
            plt.plot(audio[:feature_length * self.hp.Sound.Frame_Shift])
    ...
    def Inference_Epoch(self):
        ...
        batch_size = self.hp.Inference_Batch_Size or self.hp.Train.Batch_Size
        for step, (features, feature_lengths, sources) in tqdm(
            enumerate(self.dataloader_dict['Inference']),
            desc='[Inference]',
            total= math.ceil(len(self.dataloader_dict['Inference'].dataset) / batch_size)
            ):
            self.Inference_Step(features, feature_lengths, sources, start_index= step * batch_size)
